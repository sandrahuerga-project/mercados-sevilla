# Guiones de los flujos

Los 23 flujos de WhatsApp en texto plano, para revisar el copy de un vistazo.

**Esto es una copia generada.** El original son los JSON de
`prototype/src/flows/`. Para corregir un texto se edita el JSON y se regenera:

```
cd prototype
node scripts/exportar-guiones.mjs
```

Las ramas de los botones van sangradas: cada `**Si toca \`X\`:**` abre el
camino que sale de ese botón.

## carmen

- [C01 — Darse de alta](c01.md)
- [C02 — El saludo de cada mañana](c02.md)
- [C03 — Pedir mandando un audio](c03.md)
- [C07 — Seguir el pedido hasta recogerlo](c07.md)
- [C08 — Cuando se agota un producto](c08.md)
- [C11 — Hablar con el placero](c11.md)

## david

- [C04 — Pedir sin esperar al vídeo](c04.md)
- [C05 — Comprar en dos puestos a la vez](c05.md)
- [C06 — Repetir el pedido de siempre](c06.md)
- [C09 — Cambiar algo del pedido](c09.md)
- [C10 — Cancelar antes de que lo acepten](c10.md)
- [C12 — Cancelar con el pedido en marcha](c12.md)

## antonio

- [P01 — Dar de alta el puesto](p01.md)
- [P02 — Mandar el vídeo del día](p02.md)
- [P03 — Resolver un pedido](p03.md)
- [P04 — Cerrar el día](p04.md)

## limite

- [S01 — Pedir con el puesto cerrado](s01.md)
- [S02 — El cliente no recoge](s02.md)
- [S03 — Cliente bloqueado](s03.md)
- [S04 — El bot no entiende el pedido](s04.md)
- [S05 — Se sale a mitad del alta](s05.md)
- [S06 — Vive fuera de la zona de reparto](s06.md)
- [S07 — El placero no manda el vídeo](s07.md)
